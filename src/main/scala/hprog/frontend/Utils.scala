...
///////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  def updateSyntax(prog:Syntax, varsDcl:Map[String,Expr],
                   iteration:Int, varsDifEqs:List[List[String]],
                   control:Int):(Syntax,Map[String,Expr],Int)={
   prog match {
    case Atomic(as,de) => {
      var listAcum:List[Assign]=List()
     // var aux:(Assign,Map[String,NotLin])=updateSyntax(as(0),varsDcl,iteration,varsDifEqs,0)
      var aux1:Map[String,Expr]=varsDcl
    ...
    case ITE(ifP,thenP,elseP) =>  {
      var choice:Map[String,Expr]=Map()
      var auxifp=updateSyntax(ifP,varsDcl,iteration,varsDifEqs,0)
      var bol:Boolean=Eval(Map[String,Double](),auxifp._1)
  ...
  def updateSyntax(prog:Expr, varsDcl:Map[String,Expr], iteration:Int, varsDifEqs:List[List[String]], control:Int):(Expr,Map[String,Expr],Int)={
   prog match {
    case Var(v)=> {
      if (control==1) {
  ...
  def updateSyntax(prog:Assign, varsDcl:Map[String,Expr], iteration:Int, varsDifEqs:List[List[String]], control:Int):(Assign,Map[String,Expr],Int)={
   prog match {
    case Assign(v,e) => {
      //println("old:",varsDcl)
  ...
  def updateSyntax(prog:DiffEq, varsDcl:Map[String,Expr], iteration:Int, varsDifEqs:List[List[String]], control:Int):(DiffEq,Map[String,Expr],Int)={
    prog match {
    case DiffEq(v,e) => {
  ...
  def updateSyntax(prog:DiffEqs, varsDcl:Map[String,Expr], iteration:Int, varsDifEqs:List[List[String]], control:Int):(DiffEqs,Map[String,Expr],Int)={
   prog match {
  ...
  def updateSyntax(prog:Cond, varsDcl:Map[String,Expr], iteration:Int, varsDifEqs:List[List[String]], control:Int):(Cond,Map[String,Expr],Int)={
   prog match {
    case BVal(b)=> {
  ...
  */
  ...
  def extract_VarsEqs(notlin:Expr, listOfVars:List[String]):(List[Expr],Set[String]) = notlin match {
...
//  def varsEqs_calculation(list:List[Expr], set:Set[String]):Double = {
...
//  }
  ...
  def extractTotalVarsLinearExp(notlin:Expr):Int = notlin match {
   ...
   def funcTotalextract(s:String,list:List[Expr]):Int = (s,list) match {
    case ("PI",List()) => 0
    case ("E",List()) => 0
  ...
  // NON LINEAR EXPRESSIONS ONLY
   ...
   def extractVarsLinearExp(notlin:Expr, listOfVars:List[String]):Int = notlin match {
  ...
  //new
   def funcextract(s:String, list:List[Expr], listOfVars:List[String]):Int = (s,list) match {
    case ("PI",List()) => 0
    case ("E",List()) => 0
    case ("max",List(n1,n2)) => math.max(extractVarsLinearExp(n1,listOfVars),extractVarsLinearExp(n2,listOfVars))
    case ("min",List(n1,n2)) => math.max(extractVarsLinearExp(n1,listOfVars),extractVarsLinearExp(n2,listOfVars))
  ...
  def multOfPiOn2(number: Double): Boolean = {
    ...
    def calc_doubles(notlin:Expr):Double = notlin match {
      case Var(v) => 1
      case Value(v) => v
  ...
  def vars_in_min_max(nl:Expr):Double= nl match {
    case Var(v) => 0
    case Value(v) => 0
  ...
  def verifyUnsupFuncNotLin(prog:Expr):List[String]= prog match {
    case Var(v) => List()
    case Value(v) => List()
...
*/
    ...
    // new
  def getVars(notlin: Expr): Set[String] = notlin match {
    case Var(v) => Set(v)
    case Value(_) => Set()
    case Add(l1, l2) => getVars(l1) ++ getVars(l2)
    case Mult(l1, l2) => getVars(l1) ++ getVars(l2)
  ...
  def getVarsAux(list:List[Expr]): Set[String] = list match {
    case List() => Set()
...
/*
  ...
  */
    ...
    // new
  def getVarsList(notlin: Expr): List[String] = notlin match {
    case Var(v) => List(v)
    case Value(_) => List()
    case Add(l1, l2) => getVarsList(l1) ++ getVarsList(l2)
    case Mult(l1, l2) => getVarsList(l1) ++ getVarsList(l2)
  ...
  def getVarsListAux(list:List[Expr]): List[String] = list match {
    case List() => List()
...
/** Convert a list of assignments to a Valuation, i.e., to a Map[String,SyExpr]. */
  def toValuation(as:List[Assign],prev:ValuationSyExpr): ValuationSyExpr = {
...
//////////////////////////////////////////////////////////////////////////////////////
  ...
  def exprVarToExpr(e:SyExprVar,prev:ValuationSyExpr): SyExpr = e match {
